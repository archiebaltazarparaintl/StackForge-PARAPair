...
const APPLICANTS = [
...

  {
    ...
    rating: 4.8,
    bio: 'Loves typography and helping startups shine through clever branding.',
    skills: [
      'Branding',
      'Typography',
      'Figma',
...
const COMPANIES = [
  {
    ...
    rating: 5.0,
    bio: 'We forge high-scale cloud solutions for global innovators.',
    skills: [
      'Cloud',
      'AI',
      'Security',
    ...
    >
      <div className="relative h-[320px] pointer-events-none">
        ...
        <div className="absolute top-5 left-5 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full flex items-center gap-1">
          ...
          <Star
            size={10}
            className="fill-[#0EA5A5] text-[#0EA5A5]"
          />
        </div>
      </div>

      <div className="p-7 flex flex-col gap-2">
        <h3 className="text-2xl font-[900] text-[#0D1B2A] tracking-tighter leading-none">
          {item.name}
        </h3>

        <p className="text-[#0EA5A5] font-extrabold text-[12px] uppercase tracking-wider">
          {item.role}
        </p>
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          {item.skills.map(
            (skill: string) => (
              <span
                key={skill}
                className="px-3 py-1 bg-[#F3F6F9] text-[#0D1B2A] rounded-full text-[9px] font-black uppercase tracking-[0.1em] border border-slate-100"
              >
                {skill}
              </span>
            ),
        ...
        <AnimatePresence mode="popLayout">
          {item ? (
            ...
            <motion.div
              ...
              className="absolute inset-0 flex flex-col items-center justify-center text-center p-12 bg-white rounded-[40px] border-2 border-dashed border-slate-200"
            >
              <RotateCcw
                size={40}
                className="text-[#0EA5A5] mb-4 cursor-pointer"
                onClick={() =>
                  setIndex(0)
                }
              />
